...
def search():
    ...
    if details.get(1.0, END) != "":
        ...
        details.insert(1.0,
                       ...
                       f"   followers : {data['graphql']['user']['edge_followed_by']['count']}"
                       f"      following : {data['graphql']['user']['edge_follow']['count']} \n "
                       f"  full name : {data['graphql']['user']['full_name']} \n"
                       f" Total post : {data['graphql']['user']['edge_owner_to_timeline_media']['count']} "
                       f" category : {data['graphql']['user']['category_enum']} \n        "
                       f"Email : {data['graphql']['user']['business_email']} \n"
                       f"bio-link:{data['graphql']['user']['external_url']}"
                       f"private account:{data['graphql']['user']['is_private']} ||"
                       f" verified account:{data['graphql']['user']['is_verified']}"
                       f"  bussiness account:{data['graphql']['user']['is_business_account']}  \n \n   see profile picture")
